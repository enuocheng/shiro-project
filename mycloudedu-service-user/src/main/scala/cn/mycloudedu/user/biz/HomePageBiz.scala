...
class HomePageBiz {
  ...
  def hotCompany(pageOrder: PageOrder): PageResult[JavaMap[String,Any]] = {
    ...
    val list: JavaList[JavaMap[String,Any]] = companyMapper.hotCompany()
    val pageInfo = new PageInfo[JavaMap[String,Any]](list)
    val pageResult = new PageResult[JavaMap[String,Any]](pageOrder, pageInfo)
    pageResult
  }
