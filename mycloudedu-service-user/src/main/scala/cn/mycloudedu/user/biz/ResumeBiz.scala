...
class ResumeBiz {
  ...
  def getResumeSample(userId: Int): JavaMap[String, Any] = {
    resumeMapper.getResumeSample(userId)
  }
  ...
  def setResumeSample(userId: Int, param: ResumeSampleParam): Unit = {
    ...
    resume.setUpdateTime(DateUtil.getTimeStamp)
    this.getResumeBase(userId) match {
      case Some(resumeBase) => resume.setSampleDescription(param.getSampleDescription)
        resume.setSampleName(param.getSampleName)
        resume.setSampleFileUrl(param.getSampleFileUrl)
        if (resumeBase.getSamplePercent == 0)
          resume.setSamplePercent(ResumeConstant.SAMPLE_DESCRIPTION + ResumeConstant.SAMPLE_FILE_URL
            + ResumeConstant.SAMPLE_NAME)
        resume.setId(resumeBase.getId)
        resumeMapper.updateByPrimaryKeySelective(resume)
      case None => resume.setCreateTime(DateUtil.getTimeStamp)
        resume.setUserId(userId)
        resume.setSampleDescription(param.getSampleDescription)
        resume.setSampleName(param.getSampleName)
        resume.setSampleFileUrl(param.getSampleFileUrl)
        resume.setSamplePercent(ResumeConstant.SAMPLE_DESCRIPTION + ResumeConstant.SAMPLE_FILE_URL
          + ResumeConstant.SAMPLE_NAME)
